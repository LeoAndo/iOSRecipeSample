...
- (void)application:(UIApplication *)application didRegisterForRemoteNotificationsWithDeviceToken:(NSData *)deviceToken {
    // 通知を既に表示している場合は、デバイストークンを表示しないようにする
    if (!notificationIsShown) {
        // 取得したデバイストークンを16進数に変換
        ...
        self.message = [NSString stringWithFormat:@"このデバイスのデバイストークンは%@です。\nリモート通知が発生したらメッセージを表示します。", deviceTokenString];
        ViewController *viewController = (ViewController *)self.window.rootViewController;
        viewController.messageTextView.text = self.message;
    }
}
...
{
    self.message = [NSString stringWithFormat:@"リモート通知への登録に失敗したため、リモート通知を受け取ることができません。\nエラーコード %ld (%@)", (long)[error code], [error localizedDescription]];
    ViewController *viewController = (ViewController *)self.window.rootViewController;
    viewController.messageTextView.text = self.message;
}
